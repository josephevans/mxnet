...
NOSE_ARGUMENTS="$NOSE_COVERAGE_ARGUMENTS $NOSE_TIMER_ARGUMENTS --verbose"

CI_CUDA_COMPUTE_CAPABILITIES="-gencode=arch=compute_52,code=sm_52 -gencode=arch=compute_70,code=sm_70"
CI_CMAKE_CUDA_ARCH="5.2 7.0"

# for g3 instances
#CI_CUDA_COMPUTE_CAPABILITIES="-arch=sm_50 -gencode=arch=compute_50,code=sm_50"
#CI_CMAKE_CUDA_ARCH="5.2"
...
build_wheel() {
    ...
    PYTHON_DIR=${1:-$DRONE_WORKSPACE/python}
    BUILD_DIR=${2:-$DRONE_WORKSPACE/build}
...
build_ubuntu_cpu_release() {
    set -ex
    cd $DRONE_WORKSPACE/build
    cmake \
        -DUSE_MKL_IF_AVAILABLE=OFF \
        -DUSE_MKLDNN=OFF \
        -DUSE_CUDA=OFF \
        -G Ninja $DRONE_WORKSPACE
    ninja -j$NUMPROC
}
...
build_jetson() {
    ...
    build_wheel $DRONE_WORKSPACE/python $DRONE_WORKSPACE/lib
    popd
}
...
build_ubuntu_cpu_clang_tidy() {
    ...
    ninja -j$NUMPROC
    cd $DRONE_WORKSPACE
    $CLANG_TIDY -p $DRONE_WORKSPACE/build -j $NUMPROC -clang-tidy-binary clang-tidy-6.0 $DRONE_WORKSPACE/src
}
...
build_ubuntu_gpu_tensorrt() {
    ...
    mkdir -p $DRONE_WORKSPACE/lib/
    cp 3rdparty/onnx-tensorrt/third_party/onnx/build/*.so $DRONE_WORKSPACE/lib/
    cp -L 3rdparty/onnx-tensorrt/build/libnvonnxparser_runtime.so.0 $DRONE_WORKSPACE/lib/
    cp -L 3rdparty/onnx-tensorrt/build/libnvonnxparser.so.0 $DRONE_WORKSPACE/lib/
...
build_ubuntu_gpu_cuda101_cudnn7_mkldnn_cpp_test() {
    ...
    make \
        ...
        USE_SIGNAL_HANDLER=1                      \
        -j$NUMPROC
    make test USE_CPP_PACKAGE=1 -j$NUMPROC
    make cython PYTHON=python3
}
...
cd_unittest_ubuntu() {
    ...
    $nose_cmd $NOSE_TIMER_ARGUMENTS tests/python/unittest
    $nose_cmd $NOSE_TIMER_ARGUMENTS tests/python/quantization
    ...
    if [[ ${mxnet_variant} = cu* ]]; then
        $nose_cmd $NOSE_TIMER_ARGUMENTS tests/python/gpu
    ...
    if [[ ${mxnet_variant} = *mkl ]]; then
        ...
        if [[ ${python_cmd} = "python3" ]]; then
            $nose_cmd $NOSE_TIMER_ARGUMENTS tests/python/mkl
        fi
    fi
}
...
unittest_ubuntu_python3_cpu() {
    ...
    export MXNET_ENABLE_CYTHON=0
    export DMLC_LOG_STACK_TRACE_DEPTH=10
    nosetests-3.4 $NOSE_ARGUMENTS --with-xunit --xunit-file nosetests_unittest.xml tests/python/unittest
    nosetests-3.4 $NOSE_ARGUMENTS --with-xunit --xunit-file nosetests_quantization.xml tests/python/quantization
}
...
unittest_ubuntu_python3_cpu_mkldnn() {
    ...
    export DMLC_LOG_STACK_TRACE_DEPTH=10
    nosetests-3.4 $NOSE_ARGUMENTS --with-xunit --xunit-file nosetests_unittest.xml tests/python/unittest
    nosetests-3.4 $NOSE_ARGUMENTS --with-xunit --xunit-file nosetests_mkl.xml tests/python/mkl
}
...
unittest_ubuntu_python3_gpu() {
    ...
    export MXNET_ENABLE_CYTHON=0
    export DMLC_LOG_STACK_TRACE_DEPTH=10
    nosetests-3.4 $NOSE_ARGUMENTS --with-xunit --xunit-file nosetests_gpu.xml tests/python/gpu
}
...
unittest_ubuntu_tensorrt_gpu() {
    ...
    export MXNET_STORAGE_FALLBACK_LOG_VERBOSE=0
    export MXNET_SUBGRAPH_VERBOSE=0
    export LD_LIBRARY_PATH=$DRONE_WORKSPACE/lib:$LD_LIBRARY_PATH
    export CUDNN_VERSION=${CUDNN_VERSION:-7.0.3}
    export MXNET_ENABLE_CYTHON=0
    export DMLC_LOG_STACK_TRACE_DEPTH=10
    tests/python/tensorrt/lenet5_train.py
    nosetests-3.4 $NOSE_ARGUMENTS --with-xunit --xunit-file nosetests_trt_gpu.xml --nocapture tests/python/tensorrt/
}
...
unittest_ubuntu_python3_quantization_gpu() {
    ...
    export MXNET_ENABLE_CYTHON=0
    export DMLC_LOG_STACK_TRACE_DEPTH=10
    nosetests-3.4 $NOSE_ARGUMENTS --with-xunit --xunit-file nosetests_quantization_gpu.xml tests/python/quantization_gpu
}
...
unittest_ubuntu_cpu_R() {
    ...
    mkdir -p ~/.R/
    echo  "MAKEFLAGS = -j$NUMPROC" > ~/.R/Makevars
...
unittest_ubuntu_cpu_julia() {
    ...
    export LD_LIBRARY_PATH=$DRONE_WORKSPACE/build:$LD_LIBRARY_PATH
...
unittest_ubuntu_cpu_julia07() {
    set -ex
    unittest_ubuntu_cpu_julia /usr/local/julia07
}

unittest_ubuntu_cpu_julia10() {
    set -ex
    unittest_ubuntu_cpu_julia /usr/local/julia10
}

unittest_centos7_cpu() {
    set -ex
    cd $DRONE_WORKSPACE
    python3.6 -m "nose" $NOSE_ARGUMENTS --with-xunit --xunit-file nosetests_unittest.xml tests/python/unittest
    python3.6 -m "nose" $NOSE_ARGUMENTS --with-xunit --xunit-file nosetests_train.xml tests/python/train
}
...
unittest_centos7_gpu() {
    ...
    export CUDNN_VERSION=${CUDNN_VERSION:-7.0.3}
    export DMLC_LOG_STACK_TRACE_DEPTH=10
    python3.6 -m "nose" $NOSE_ARGUMENTS --with-xunit --xunit-file nosetests_gpu.xml tests/python/gpu
}
...
integrationtest_ubuntu_gpu_caffe() {
    set -ex
    export PYTHONPATH=/work/deps/caffe/python:$PYTHONPATH
    export DMLC_LOG_STACK_TRACE_DEPTH=10
...
integrationtest_ubuntu_cpu_asan() {
    ...
    cd $DRONE_WORKSPACE/build/cpp-package/example/
    $DRONE_WORKSPACE/cpp-package/example/get_data.sh
    ./mlp_cpu
}
...
test_ubuntu_cpu_python3() {
    set -ex
    pushd .
    export MXNET_LIBRARY_PATH=$DRONE_WORKSPACE/build/libmxnet.so
    export DMLC_LOG_STACK_TRACE_DEPTH=10
    VENV=mxnet_py3_venv
    virtualenv -p `which python3` $VENV
    source $VENV/bin/activate

    cd $DRONE_WORKSPACE/python
    pip3 install nose nose-timer
    pip3 install -e .
    cd $DRONE_WORKSPACE
    python3 -m "nose" $NOSE_ARGUMENTS tests/python/unittest
...
nightly_test_rat_check() {
    ...
    exec 5>&1
    OUTPUT=$(java -jar apache-rat-0.13-SNAPSHOT.jar -E $DRONE_WORKSPACE/tests/nightly/apache_rat_license_check/rat-excludes -d $DRONE_WORKSPACE|tee >(cat - >&5))
...
nightly_test_imagenet_inference() {
    ...
    export DMLC_LOG_STACK_TRACE_DEPTH=10
    echo $PWD
    cp $DRONE_WORKSPACE/build/cpp-package/example/imagenet_inference $DRONE_WORKSPACE/cpp-package/example/inference/
    cd $DRONE_WORKSPACE/cpp-package/example/inference/
...
nightly_test_javascript() {
    ...
    ./emcc
    touch ~/.emscripten
    make -C $DRONE_WORKSPACE/amalgamation libmxnet_predict.js MIN=1 EMCC=/work/deps/emscripten/emcc
}

#Tests Model backwards compatibility on MXNet
nightly_model_backwards_compat_test() {
    set -ex
    export PYTHONPATH=$DRONE_WORKSPACE/python/
    export DMLC_LOG_STACK_TRACE_DEPTH=10
...
nightly_straight_dope_python3_single_gpu_tests() {
    set -ex
    cd $DRONE_WORKSPACE/tests/nightly/straight_dope
    export PYTHONPATH=$DRONE_WORKSPACE/python/
...
nightly_tutorial_test_ubuntu_python3_gpu() {
    ...
    cd $DRONE_WORKSPACE/tests/tutorials
...
nightly_java_demo_test_cpu() {
    set -ex
    cd $DRONE_WORKSPACE/scala-package/mxnet-demo/java-demo
...
nightly_scala_demo_test_cpu() {
    set -ex
    cd $DRONE_WORKSPACE/scala-package/mxnet-demo/scala-demo
    mvn -B -Pci-nightly install
    bash bin/demo.sh
    bash bin/run_im.sh
}
...
nightly_estimator() {
    set -ex
    export DMLC_LOG_STACK_TRACE_DEPTH=10
    cd $DRONE_WORKSPACE/tests/nightly/estimator
    export PYTHONPATH=$DRONE_WORKSPACE/python/
...
build_docs_setup() {
    ...
    mxnetlib_folder="$DRONE_WORKSPACE/lib"
...
build_python_docs() {
   ...
   pip install -e $DRONE_WORKSPACE/python --user
   ...
   mv python/python-artifacts.tgz $DRONE_WORKSPACE/docs/_build/
   popd
...
cd_integration_test_pypi() {
    ...
    # execute tests
    ${python_cmd} $DRONE_WORKSPACE/tests/python/train/test_conv.py ${test_conv_params}
    ${python_cmd} $DRONE_WORKSPACE/example/image-classification/train_mnist.py ${mnist_params}
}
